<template>
  <div class="order">
    ...
    <div class="order__list order__list__title row col-sm-11">
      <div class="order__list__item col-sm-5">Sản phẩm</div>
    ...
    <div
      ...
      class="order__list row col-sm-11"
      :key="item.id"
    >
      <div class="order__list__item order__list__item--sach col-sm-5">
        ...
        >
          ...
          />
          <div>
            <p class="product-name">{{ item_item.sach.ten }}</p>
            <p>
              x{{ item_item.soluong }} -
              {{ item_item.gia.toLocaleString() }} VNĐ
            </p>
          </div>
        </div>
      </div>
      <div class="order__list__item col-sm-2">
      ...
      <div
        ...
        class="order__list__item col-sm-1 text-success"
      >
        {{ item.trangthai }}
      </div>
      <div
        v-else-if="item.trangthai == 'Quá hạn'"
        class="order__list__item col-sm-1 text-danger"
      >
        {{ item.trangthai }}
      </div>
