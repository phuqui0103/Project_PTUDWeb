  ...
  <div class="order">
    ...
    <div class="order__form" :class="{ 'order__form--active': order_form }">
      <form action="" @submit.prevent="handleSubmit">
        ...
        <div
          class="order__form__item order__form__item__book"
...
export default {
  ...
  methods: {
    ...
    handlePhuthu() {
      ...
      const ngay_tra = new Date(
        parseInt(parts_two[0]),
        parseInt(parts_two[1]) - 1,
        parseInt(parts_two[2])
      );
    ...
    async handleSubmit() {
      if (!this.handlePhuthu) this.phuthu = (this.tongtien * 20) / 100;
      !this.phuthu
        ? (this.tongtien = this.tongtien)
        : (this.tongtien = this.tongtien + (this.tongtien * 20) / 100);
      const data = {
        _id: this.order._id,
...
.order__header__search input:focus,
.order__header__filter select:focus {
  border: none;
  outline: none;
}
